...
class ImdbSelect:
    ...
    def add_all(self, actors, rating_above=0, votes_above=0, manual=False):
        for actor in actors:
            ...
            movie_data_map, error_list = self.collect_movie_data(actor)
            print("Could not find: " + ', '.join(error_list))
        ...
        for imdb_id in movies:
            ...
            try:
                if not self.ask.is_answered_no(imdb_id) and not self.radarr.movie_exist(imdb_id):
                    movie_data_map[imdb_id] = self.radarr.get_movie(imdb_id)
    ...
    def choices(self, movie_data_map, rating_above=None, votes_above=None, manual=False):
        movies_to_add = []
        for imdb_id, movie in movie_data_map.items():
            if rating_above and votes_above:
                ...
                elif not movie.votes_above(votes_above):
                    print("\t\t" + str(movie) + " has not enough votes")
                else:
                    if manual:
                        if self.ask.ask(imdb_id, movie):
                            movies_to_add.append(movie)
                    else:
                        print(str(movie) + " has good ratings")
                        movies_to_add.append(movie)
            elif manual and self.ask.ask(imdb_id, movie):
                    movies_to_add.append(movie)
